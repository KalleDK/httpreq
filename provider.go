...
// with the libdns interfaces for HTTPReq.
package httpreq
...
// Provider facilitates DNS record manipulation with HTTPReq.
type Provider struct {
	// Credentials are the username and password required for authentication.
	// The fields are optional and can be omitted.
	Credentials
	// Endpoint is the URL of the HTTPReq server.
	Endpoint string `json:"endpoint"`
	// HTTPClient is the client used to communicate with the HTTPReq server.
...
func (p *Provider) doAction(ctx context.Context, endpoint *url.URL, action string, zone string, record libdns.Record) (libdns.Record, error) {
	...
	if record.Type != "TXT" {
		return libdns.Record{}, fmt.Errorf("HTTPReq provider only supports TXT records")
	}
	...
	reqBody := new(bytes.Buffer)
	{
		...
		if err := json.NewEncoder(reqBody).Encode(msg); err != nil {
			return libdns.Record{}, fmt.Errorf("HTTPReq provider could not marshal JSON: %w", err)
		}
	}
	...
	if err != nil {
		return libdns.Record{}, fmt.Errorf("HTTPReq provider could not create request: %w", err)
	}
	...
	if err != nil {
		return libdns.Record{}, fmt.Errorf("HTTPReq provider could not send request: %w", err)
	}
	...
	// Validate Response
	{
		if resp.StatusCode != http.StatusOK {
			return libdns.Record{}, fmt.Errorf("HTTPReq provider received status code %d", resp.StatusCode)
		}

		var respMsg httpReq
		if err := json.NewDecoder(resp.Body).Decode(&respMsg); err != nil {
			return libdns.Record{}, fmt.Errorf("HTTPReq provider could not unmarshal JSON: %w", err)
		}

		if respMsg.FQDN != libdns.AbsoluteName(record.Name, zone) {
			return libdns.Record{}, fmt.Errorf("HTTPReq provider received unexpected FQDN %q", respMsg.FQDN)
		}

		if respMsg.Value != record.Value {
			return libdns.Record{}, fmt.Errorf("HTTPReq provider received unexpected Value %q", respMsg.Value)
		}
	}
...
func (p *Provider) doActions(ctx context.Context, action string, zone string, records []libdns.Record) ([]libdns.Record, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("HTTPReq provider invalid endpoint [%s]: %w", p.Endpoint, err)
	}
	...
	for _, record := range records {
		...
		if err != nil {
			return completedRecords, fmt.Errorf("HTTPReq provider could not %s record %q: %w", action, record.Name, err)
		}
...
// This is not supported by the HTTPReq provider.
func (p *Provider) GetRecords(ctx context.Context, zone string) ([]libdns.Record, error) {
	return nil, fmt.Errorf("HTTPReq provider does not support listing records")
}

// AppendRecords adds records to the zone. It returns the records that were added.
// This is not supported by the HTTPReq provider.
func (p *Provider) AppendRecords(ctx context.Context, zone string, records []libdns.Record) ([]libdns.Record, error) {
	return nil, fmt.Errorf("HTTPReq provider does not support appending records")
}
